...
function safeAmount(amt) {
  ...
  let cleaned = String(amt).replace(/[¥￥,，]/g, '').replace(/元$/g, '').trim();
 ...
 */
export function genInvoiceName(data, filename) {
  const ext = extractExt(filename);

  const date     = safeDate(data && data.date);
  const supplier = safeStr(data && data.supplier, 20) || '';
  const buyer    = safeStr(data && data.buyer, 20)    || '';
  const amount   = safeAmount(data && data.amount);

  // 严格按 Python 格式拼接
  let name = `${date}_${supplier}_${buyer}_${amount}元${ext}`;
 ...
 *   '_'.join(parts) + ext
 */
export function genTrainName(data, filename) {
  const ext = extractExt(filename);

  const date  = safeDate(data && data.date);
  const fromS = safeStr(data && data.from_station, 10);
  const toS   = safeStr(data && data.to_station, 10);
  const price = safeAmount(data && data.price);
...
function abbreviateParty(name) {
  ...
  for (const sfx of SUFFIXES) {
    if (name.endsWith(sfx)) {
      const candidate = name.slice(0, name.length - sfx.length);
...
export function genContractName(data, filename) {
  const ext = extractExt(filename);

  const date          = safeDate(data && data.sign_date);
  const contractName  = safeStr(data && data.contract_name, 15) || '合同';
  const partyA        = abbreviateParty(data && data.party_a);
  const partyB        = abbreviateParty(data && data.party_b);
  ...
  if (amountRaw !== null && amountRaw !== undefined && amountRaw !== '') {
    const n = parseFloat(String(amountRaw).replace(/[¥￥,，]/g, '').replace(/元$/g, ''));
    if (!isNaN(n) && n > 0) {
      const amtStr = (n === Math.floor(n)) ? `${Math.floor(n)}元` : `${n.toFixed(2)}元`;
      parts.push(amtStr);
    }
  }
...
export function genAnyFilename(data, docType, filename) {
