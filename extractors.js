...
(function (global) {
      ...
      fetch('https://api.ocr.space/parse/image', { method: 'POST', body: form })
        ...
        .then(data => {
          if (data && data.ParsedResults && data.ParsedResults.length > 0) {
            resolve(data.ParsedResults.map(p => p.ParsedText || '').join('\n'));
          } else {
            resolve('');
          }
        })
  ...
  function pdfToText(file) {
    return new Promise((res, rej) => {
      ...
      file.arrayBuffer().then(buf => {
        global.pdfjsLib.getDocument({ data: buf }).promise.then(pdf => {
          let pages = Math.min(pdf.numPages, 8),
              text = '',
              idx = 1;
          function next() {
  ...
  function extractInvoice(text) {
    ...
    let b = text.match(/购买方\s*(?:名称)?[：:]\s*([^\n]{2,30})/); if (b) r.buyer = b[1].trim();
    let s = text.match(/销售方\s*(?:名称)?[：:]\s*([^\n]{2,30})/); if (s) r.supplier = s[1].trim();
    let am = text.match(/价税合计[^\d]*[¥￥]?\s*(\d+\.\d{2})/); if (!am) am = text.match(/[¥￥]\s*(\d+\.\d{2})/); if (am) r.amount = parseFloat(am[1]).toFixed(2);
    return r;
  }
  ...
  global.EX = { extractFromFile, extractFromFilename, detectType };
})(window);
